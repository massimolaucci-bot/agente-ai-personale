...
def load_memory():
    if _supabase_client:
        try:
            res = (
                _supabase_client.table("chat_messages")
                .select("role,content")
                .order("id", desc=False)
...
def save_memory(messages):
    if _supabase_client:
        try:
            trimmed = messages[-MAX_SAVED_MESSAGES:]
            _supabase_client.table("chat_messages").delete().gte("id", 0).execute()
            if trimmed:
...
def load_knowledge():
    if _supabase_client:
        try:
            res = (
                _supabase_client.table("knowledge_base")
                .select("text_content")
                .eq("id", 1)
                .single()
                .execute()
            )
            return res.data.get("text_content", "") if res.data else ""
        except Exception:
            pass
...
def save_knowledge(text):
    if _supabase_client:
        try:
            _supabase_client.table("knowledge_base").update(
                {"text_content": text}
            ).eq("id", 1).execute()
            return
        except Exception:
...
MEMORIA_PERSISTENTE = _supabase_client is not None
