...
MAX_HISTORY_MESSAGES = 12   # quante battute recenti mandare al modello ad ogni richiesta
MAX_MESSAGE_CHARS = 4000    # lunghezza massima di un singolo messaggio inviato al modello
...
def build_api_messages(history, knowledge_text):
    system_prompt = (
...
with st.sidebar:
    ...
    st.caption("La cronologia mostrata resta finche il server e attivo; al modello viene inviata solo la parte piu recente per evitare errori. Se Render riavvia il servizio, tutto si azzera.")
    ...
    st.caption("Scrivi qui cosa Carpanet AI deve sempre sapere o come si deve comportare: resta valido anche se la chat si azzera. Nota: su questo piano Render gratuito, se il servizio si riavvia per inattivita, anche queste istruzioni vengono perse finche non le salvi di nuovo.")
...
if not groq_api_key:
    ...
    (function() {
        function injectVoiceUI() {
            ...
            if (inputContainer) {
                inputContainer.style.border = '1.5px solid #00e5ff';
                inputContainer.style.borderRadius = '24px';
                inputContainer.style.background = 'rgba(255,255,255,0.04)';
                inputContainer.style.alignItems = 'center';
            }

            // Riporta lo sfondo della barra inferiore fissa allo stesso colore della pagina
            var node = chatInput.parentElement;
            for (var i = 0; i < 6 && node; i++) {
                node.style.background = 'linear-gradient(135deg, #0a0e17 0%, #10162a 100%)';
    ...
    if prompt := st.chat_input("Chiedimi qualcosa o usa il microfono accanto al campo..."):
        ...
        with st.chat_message("assistant"):
            try:
                ...
                err_msg = str(e)
                if "413" in err_msg or "too large" in err_msg.lower() or "request_too_large" in err_msg.lower():
                    st.error("La richiesta era troppo grande per il modello. Ho gia ridotto la cronologia inviata per i prossimi messaggi: riprova a scrivere di nuovo.")
                else:
